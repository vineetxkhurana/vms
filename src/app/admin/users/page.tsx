    ...
    <>
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          >
            Customers{' '}
            <span className="text-on-surface-muted text-base font-normal">({users.length})</span>
          </h1>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap mb-8">
        ...
        <div className="relative flex-1" style={{ minWidth: 200 }}>
          <Icon
            name="search"
            className="text-[18px] text-on-surface-muted"
          ...
          {TIERS.map(t => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                cursor: 'pointer',
                background: filter === t ? TIER_CONFIG[t].bg : 'rgba(10,20,45,0.7)',
                color: filter === t ? TIER_CONFIG[t].color : '#8fafc7',
                border: `1px solid ${filter === t ? TIER_CONFIG[t].color + '50' : 'rgba(0,194,255,0.2)'}`,
              }}
            >
              {TIER_CONFIG[t].label}
            </button>
          ...
          return (
            ...
            >
              ...
              <td className="px-5 py-4 text-on-surface-muted text-xs">
                {new Date(u.created_at * 1000).toLocaleDateString('en-IN', {
                  day: 'numeric',
                  month: 'short',
                  year: 'numeric',
                })}
              ...
              <td className="px-5 py-4">
                <select
                  value={u.role}
                  onChange={e => updateTier(u.id, e.target.value as UserRole)}
