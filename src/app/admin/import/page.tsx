    ...
    return lines
      ...
      .map((line, i) => {
        ...
        if (isNaN(stockNum) || isNaN(priceNum))
          throw new Error(`Row ${i + 2}: stock and price must be numbers`)
    ...
    <div className="max-w-5xl mx-auto py-8 px-4">
      ...
      <div className="glass rounded-2xl p-6 mb-6">
        ...
        <ol className="text-sm text-on-surface-muted space-y-1 list-decimal list-inside">
          ...
          <li>
            Make sure it has these columns: <code className="text-primary">name, stock, price</code>
          </li>
          <li>
            Optional columns:{' '}
      ...
      >
        ...
        <p className="text-sm text-on-surface-muted">
          Supports .csv files exported from any inventory software
        </p>
        <input
          ref={fileRef}
          type="file"
        ...
        <div className="glass rounded-2xl overflow-hidden">
          ...
          >
            ...
            >
              {importing ? (
                <>
                  <Icon name="autorenew" className="animate-spin text-[16px]" /> Importing…
                </>
              ) : (
                <>
                  <Icon name="upload" className="text-[16px]" /> Confirm Import
                </>
              )}
            </button>
          </div>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(0,194,255,0.1)' }}>
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-xs font-semibold text-on-surface-muted uppercase tracking-wider"
                    >
                      {h}
                    </th>
                  ))}
