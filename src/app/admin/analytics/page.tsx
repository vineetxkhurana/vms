  ...
  const load = useCallback(() => {
    ...
    adminFetch(`/api/admin/analytics?days=${days}`)
      .then(async r => {
        ...
        if (!r.ok) {
          setError((d as any).error ?? 'Failed')
          return
        }
  ...
  return (
    <>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="skeleton rounded-2xl" style={{ height: 120 }} />
          ))
        ) : (
          <>
            {(['product_view', 'add_to_cart', 'order_placed', 'user_registered'] as const).map(
              ev => {
                ...
                return (
                  <div key={ev} className="glass rounded-2xl p-5">
                    ...
                    <p className="font-display font-black text-on-surface text-2xl">
                      {fmt(row?.count ?? 0)}
                    </p>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        <div className="glass rounded-2xl p-6">
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {(['product_view', 'add_to_cart', 'checkout_started', 'order_placed'] as const).map(
                (ev, idx, arr) => {
                  const cfg = EVENT_LABELS[ev]
                  const count = data?.funnel.find(f => f.event === ev)?.count ?? 0
                  const prev =
                    idx > 0 ? (data?.funnel.find(f => f.event === arr[idx - 1])?.count ?? 0) : count
                  ...
                  return (
                    <div key={ev}>
                      ...
                      >
                        <span style={{ fontSize: 13, color: '#e8f4fd', fontWeight: 500 }}>
                          {cfg.label}
                        </span>
        ...
        <div className="glass rounded-2xl p-6">
          <h2 className="font-display font-bold text-on-surface text-base mb-5">
            Daily Orders ({days}d)
          </h2>
          {loading ? (
            <div className="skeleton rounded-xl" style={{ height: 160 }} />
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: `${h}%`,
                        borderRadius: 4,
                        background: 'linear-gradient(180deg,#00c2ff,#7c3aed)',
                        minHeight: 4,
                      }}
                    />
                    <span
      ...
      {data && data.top_products.length > 0 && (
        <div className="glass rounded-2xl p-6">
          <h2 className="font-display font-bold text-on-surface text-base mb-4">
            Top Viewed Products
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {data.top_products.map((p, i) => (
              ...
              >
                <span style={{ fontSize: 13, fontWeight: 800, color: '#00c2ff', width: 20 }}>
                  #{i + 1}
                </span>
                <span style={{ flex: 1, fontSize: 13, color: '#e8f4fd' }}>
                  Product #{p.product_id}
                </span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#00c2ff' }}>
                  {fmt(p.count)} views
                </span>
              </div>
            ))}
      ...
      {!loading && data?.total_events === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: '#8fafc7' }}>
          ...
          <p style={{ fontSize: 13, marginTop: 8 }}>
            Events will appear here once customers start visiting the store.
          </p>
        </div>
      )}
    </>
