  ...
  const load = useCallback(() => {
    ...
    adminFetch('/api/admin/stats', { cache: 'no-store' } as RequestInit)
      .then(async r => {
        ...
        if (!r.ok) {
          setError((d as any).error ?? 'Failed to load stats')
          return
        }
        setStats(d as Stats)
      })
        ...
        {loading ? (
          ...
          <>
            <Stat
              ...
              value={fmt(stats.revenue_paise)}
              color="#00c2ff"
              sub={
                stats.pending_paise > 0
                  ? `+ ${fmt(stats.pending_paise)} pending`
                  : 'paid orders only'
              }
            />
            <Stat
              ...
              return (
                <Link key={s} href={`/admin/orders?status=${s}`} style={{ textDecoration: 'none' }}>
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#8fafc7' }}>
                      {cfg.label}
                    </span>
                  </div>
                </Link>
        ...
        {stats && (
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link
                href="/admin/orders"
                style={{ fontSize: 12, color: '#00c2ff', textDecoration: 'none', fontWeight: 600 }}
              >
                View all →
              </Link>
            </div>
                ...
                {stats.recent_orders.map(o => (
                  ...
                  >
                    <div>
                      ...
                      <span
                        style={{ marginLeft: 10, fontSize: 13, color: '#e8f4fd', fontWeight: 500 }}
                      >
                        {o.customer_name}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: '#e8f4fd' }}>
                        {fmt(o.total_paise)}
                      </span>
              ...
              {stats.low_stock_items.map(p => (
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${p.stock === 0 ? 'rgba(239,68,68,0.2)' : 'rgba(245,158,11,0.15)'}`,
                  }}
                >
        ...
        {[
          {
            ...
            icon: 'receipt_long',
            label: 'Manage Orders',
            desc: 'Update order statuses, track shipments',
          },
          {
            href: '/admin/products',
            icon: 'inventory_2',
            label: 'Add Products',
            desc: 'Add or edit catalogue items & pricing',
          },
          {
            href: '/admin/users',
            icon: 'manage_accounts',
            label: 'Manage Customers',
            desc: 'Set customer tiers & retailer/wholesaler accounts',
          },
        ].map(({ href, icon, label, desc }) => (
          <a
            key={href}
            href={href}
            className="glass glass-hover rounded-2xl p-6 flex items-start gap-4 transition-all duration-300 hover:-translate-y-1"
