...
export async function GET(req: Request) {
  ...
  const [ordersRow, productsRow, usersRow, revenueRow, statusRow, lowStockRow, recentRow] =
    await db.batch([
      db.prepare('SELECT COUNT(*) as total FROM orders'),
      db.prepare(
        'SELECT COUNT(*) as total, SUM(CASE WHEN stock < 5 THEN 1 ELSE 0 END) as low_stock FROM products WHERE is_active=1',
      ),
      db.prepare(
        "SELECT COUNT(*) as total, SUM(CASE WHEN role='retailer' THEN 1 ELSE 0 END) as retailers, SUM(CASE WHEN role='wholesaler' THEN 1 ELSE 0 END) as wholesalers FROM users WHERE role NOT IN ('admin','staff')",
      ),
      // confirmed = payment received (paid/processing/shipped/delivered); pending = awaiting payment
      db.prepare(
        "SELECT COALESCE(SUM(CASE WHEN status NOT IN ('pending','cancelled') THEN total_paise ELSE 0 END),0) as confirmed, COALESCE(SUM(CASE WHEN status='pending' THEN total_paise ELSE 0 END),0) as pending FROM orders",
      ),
      // Orders by status
