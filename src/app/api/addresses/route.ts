...
export async function POST(req: Request) {
  ...
  if (d.is_default) {
    await db.prepare('UPDATE addresses SET is_default=0 WHERE user_id=?').bind(Number(user.sub)).run()
  }
  ...
  const result = await db
    .prepare(`INSERT INTO addresses (user_id, label, name, phone, line1, line2, city, state, pin, is_default)
              VALUES (?,?,?,?,?,?,?,?,?,?) RETURNING id`)
    .bind(Number(user.sub), d.label, d.name, d.phone, d.line1, d.line2 ?? null, d.city, d.state, d.pin, isFirst || d.is_default ? 1 : 0)
    .first<{ id: number }>()
...
export async function PATCH(req: Request) {
  ...
  // Verify ownership
  const addr = await db.prepare('SELECT id FROM addresses WHERE id=? AND user_id=?')
    .bind(id, Number(user.sub)).first<{ id: number }>()
  ...
  await db.prepare(`UPDATE addresses SET ${updates.map(([k]) => `${k}=?`).join(',')} WHERE id=? AND user_id=?`)
    .bind(...updates.map(([,v]) => v), id, Number(user.sub)).run()
...
export async function DELETE(req: Request) {
  ...
  await db.prepare('DELETE FROM addresses WHERE id=? AND user_id=?').bind(id, Number(user.sub)).run()
  return ok({ ok: true })
}
