...
export default function ProductCard({ product }: Props) {
  ...
  const options: { id: number; name: string; label: string; price: number; stock: number; image_url: string | null }[] =
    hasVariants
      ? [
          { id: product.id, name: product.name, label: product.variant_label ?? '', price: product.price, stock: product.stock, image_url: product.image_url },
          ...(product.variants ?? []).map((v: ProductVariant) => ({
            id: v.id, name: v.name, label: v.label, price: v.price, stock: v.stock, image_url: v.image_url,
          })),
        ]
      : []
  ...
  const cartProduct: Product = hasVariants
    ? { ...product, id: activeId, name: activeOption?.name ?? product.name, price: activePrice, stock: activeStock, image_url: activeImg,
  ...
  return (
    <div className="card-3d" style={{
      background: 'rgba(10,20,45,0.65)', backdropFilter: 'blur(12px)',
      border: '1px solid rgba(0,194,255,0.1)', borderRadius: 16,
      padding: '20px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden',
    }}>
      {/* Glow accent */}
      <div style={{ position: 'absolute', top: -40, right: -40, width: 120, height: 120, background: 'radial-gradient(circle, rgba(0,194,255,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
      ...
      {hasVariants && (
        <div style={{
          ...
          color: '#a78bfa', borderRadius: 100, fontSize: 10, fontWeight: 700,
          padding: '2px 8px', fontFamily: 'Inter, sans-serif', pointerEvents: 'none',
        }}>
      ...
      <Link href={`/products/${activeId}`} style={{ textDecoration: 'none' }}>
        <h4 style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: 14, color: '#e8f4fd', marginBottom: 4, lineHeight: 1.35 }}>{displayName}</h4>
      </Link>
      <p style={{ fontSize: 11, color: '#8fafc7', marginBottom: 10, fontFamily: 'Inter, sans-serif' }}>{product.category_name ?? 'General'}</p>
          ...
          {options.map(opt => (
            <button
              ...
              style={{
                ...
                background: activeOption?.id === opt.id ? 'rgba(0,194,255,0.2)' : 'rgba(0,194,255,0.05)',
                border: activeOption?.id === opt.id ? '1px solid rgba(0,194,255,0.6)' : '1px solid rgba(0,194,255,0.15)',
          ...
          {options.map(opt => (
            <button
              ...
              style={{
                ...
                fontFamily: 'Inter, sans-serif', cursor: 'pointer', transition: 'all 0.15s',
                background: activeOption?.id === opt.id ? 'rgba(124,58,237,0.25)' : 'rgba(124,58,237,0.07)',
                border: activeOption?.id === opt.id ? '1px solid rgba(124,58,237,0.6)' : '1px solid rgba(124,58,237,0.2)',
      ...
      <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        <span style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 800, fontSize: 20, color: '#00c2ff' }}>₹{priceRs}</span>
